...
local function BL6552_Update_Data_Chx(Chx)
...
local function _BL6552_Update_I_SCALE_Chx(Chx)
    I_SCALE_Chx[Chx] = 0
    if math.floor(((BL6552_Elect_IA_RMS_Chx[Chx] + 0.05) / (BL6552_Elect_IB_RMS_Chx[Chx] + 0.05)) * 100) < tonumber(fskv.get("I_SCALE_NUM_CHX_CONFIG")[Chx]) then
        I_SCALE_Chx[Chx] = 1
    end
    if math.floor(((BL6552_Elect_IB_RMS_Chx[Chx] + 0.05) / (BL6552_Elect_IA_RMS_Chx[Chx] + 0.05)) * 100) < tonumber(fskv.get("I_SCALE_NUM_CHX_CONFIG")[Chx]) then
        I_SCALE_Chx[Chx] = 1
    end
    if math.floor(((BL6552_Elect_IA_RMS_Chx[Chx] + 0.05) / (BL6552_Elect_IC_RMS_Chx[Chx] + 0.05)) * 100) < tonumber(fskv.get("I_SCALE_NUM_CHX_CONFIG")[Chx]) then
        I_SCALE_Chx[Chx] = 1
    end
    if math.floor(((BL6552_Elect_IC_RMS_Chx[Chx] + 0.05) / (BL6552_Elect_IA_RMS_Chx[Chx] + 0.05)) * 100) < tonumber(fskv.get("I_SCALE_NUM_CHX_CONFIG")[Chx]) then
        I_SCALE_Chx[Chx] = 1
    end
    if math.floor(((BL6552_Elect_IC_RMS_Chx[Chx] + 0.05) / (BL6552_Elect_IB_RMS_Chx[Chx] + 0.05)) * 100) < tonumber(fskv.get("I_SCALE_NUM_CHX_CONFIG")[Chx]) then
        I_SCALE_Chx[Chx] = 1
    end
    if math.floor(((BL6552_Elect_IB_RMS_Chx[Chx] + 0.05) / (BL6552_Elect_IC_RMS_Chx[Chx] + 0.05)) * 100) < tonumber(fskv.get("I_SCALE_NUM_CHX_CONFIG")[Chx]) then
        I_SCALE_Chx[Chx] = 1
    end
end
...
local function _BL6552_Update_ZXTO_IN_Chx(Chx)
    ...
    if BL6552_Elect_VB_RMS_Chx[Chx] < 2            then ZXTO_IN_B_Chx[Chx] = 1 end
...
local function _BL6552_Update_ZXTO_OUT_Chx(Chx)
    ...
    --------------------------------------
    -- 电压大小判断是否缺相
    if BL6552_Elect_IA_RMS_Chx[Chx] < _ZXTO_OUT_NUM then ZXTO_OUT_A_Chx[Chx] = 1 end
    if BL6552_Elect_IB_RMS_Chx[Chx] < _ZXTO_OUT_NUM then ZXTO_OUT_B_Chx[Chx] = 1 end
    if BL6552_Elect_IC_RMS_Chx[Chx] < _ZXTO_OUT_NUM then ZXTO_OUT_C_Chx[Chx] = 1 end
    --------------------------------------
end
...
local function _MQTT_Warn_I_SCALE_Chx(Chx)
    if fskv.get("I_SCALE_ENABLE_CHX_CONFIG")[Chx] == "1" then  --确实报警使能
        if I_SCALE_Chx[Chx] == 1 then
            sys.publish("DeviceWarn_Status","Alert_IS", Chx, "0", "", "")
            if fskv.get("VVVF_ENABLE_CHX_CONFIG")[Chx] == "0" then -- 非变频
...
local function _MQTT_Warn_ZXTO_IN_Chx(Chx)
    -- 先报输入缺相（电压）
    if fskv.get("ZXTO_ENABLE_CHX_CONFIG")[Chx] == "1" then -- 对应通道缺相判断使能
...
local function _MQTT_Warn_ZXTO_OUT_Chx(Chx)
    if ZXTO_IN_A_Chx[Chx] == 1 or ZXTO_IN_B_Chx[Chx] == 1 or ZXTO_IN_C_Chx[Chx] == 1 then
        -- 输出缺相（电流比例去判断）
        if fskv.get("ZXTO_ENABLE_CHX_CONFIG")[Chx] == "1" then  --确实报警使能
...
local function _MQTT_Warn_VI_OVER_Chx(Chx)
    -- 过压 过流报警
    if fskv.get("IV_NUM_ENABLE_CHX_CONFIG")[Chx] == "1" then -- 对应通道缺相判断使能
        if BL6552_Elect_VA_RMS_Chx[Chx] > fskv.get("V_NUM_CHX_CONFIG")[Chx] then
            sys.publish("DeviceWarn_Status","Alert_VF", Chx, tostring(BL6552_Elect_VA_RMS_Chx[Chx]), "", "")
            sys.publish("LED_Chx","AlertOP",Chx,0)
        end

        if BL6552_Elect_IA_RMS_Chx[Chx] > fskv.get("I_NUM_CHX_CONFIG")[Chx] then
...
local function BL6552_Mqtt_Report_Chx(Event,Chx,Data,Tag)
        -- MQTT 处理事件
        if Data == 1 then
            local _data = string.format("%.2f_%.2f_%.2f_%.2f_%.2f_%.2f_%.2f",BL6552_Data_Chx(Chx)) -- 保存数据
            sys.publish("DeviceResponse_Status",Event, Chx, _data, "1", Tag)   -----------
        elseif Data == 0 then
            sys.publish("DeviceResponse_Status",Event, Chx, "0", "0", Tag)    -----------
        end
end
